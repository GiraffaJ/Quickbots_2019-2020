...
#defining each program
    ...
    # stop all motors
        ...
        while GY.value() > 0: #this turns the blocks into the circle, need to be less than straight 90 degrees
            left_wheel_speed = -100
            right_wheel_speed = 100
            #MB is left wheel & MC is right wheel
            MB.run_forever(speed_sp=left_wheel_speed)
        ...
        MB.stop(stop_action="hold")
        MC.stop(stop_action="hold")
        tank_drive.on_for_rotations(SpeedPercent(-30), SpeedPercent(-30), 2.5) #goes forward slightly to move the block into tan
        ...
        MC.stop(stop_action="hold")
    # Drives up bridge
        tank_drive.on_for_rotations(SpeedPercent(30), SpeedPercent(30), 2.5) #drives back to base. keep speed at 50
        MB.stop(stop_action="coast")
        MC.stop(stop_action="coast")
        Launchrun()  
